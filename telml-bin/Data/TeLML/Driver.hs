...
module Data.TeLML.Driver where
...
mainWithOpts :: Options -> IO (Either Error Text.Text)
mainWithOpts options = do
  -- read the source file
  telmlSource <- case optInputFile options of
    Nothing -> getContents
    Just (InputFilePath f) -> readFile f
    Just (InputFileTest src) -> pure (BS.unpack src)
  -- attempt to parse it
...
luaMain opts docs = do
  ...
  case optTagFile opts of
    Nothing -> return ()
    Just source -> do
      status <- case source of
        InputFilePath p -> Lua.dofile (Just p)
        InputFileTest str -> Lua.dostring str
      case status of
...
data Options = Options
  { optInputFile :: Maybe InputFile,
    optUseDefaultTags :: Bool,
    optTagFile :: Maybe InputFile
  }
  deriving (Eq, Show)

data InputFile
  = InputFilePath FilePath
  | InputFileTest BS.ByteString
  deriving (Eq, Show)
...
optionDescriptions =
  [ Opt.Option
    ...
    Opt.Option
      ['t']
      ["tags"]
      (Opt.ReqArg (\f o -> o {optTagFile = Just (InputFilePath f)}) "[tagfile.lua]")
...
parseOpts = do
  ...
  case Opt.getOpt Opt.Permute optionDescriptions args of
    ...
    (flags, [input], []) ->
      return (foldl (flip id) def flags) {optInputFile = Just (InputFilePath input)}
    (_, _, errors) -> do
